...
export const assert = {
  ...
  bufferArray: (values: unknown[]): void =>
    uvu.ok(values.every((value) => value instanceof Buffer)),
  containKey: (value: object, key: string): void =>
    assert.true(Object.keys(value).includes(key)),
  containKeys: (value: object, keys: string[]): void => {
    for (const key of keys) {
      uvu.ok(value[key] !== undefined);
    }
  },
  containValues: (value: object, key: string): void =>
    assert.false(Object.values(value).includes(key)),
  defined: (value: unknown): void =>
    uvu.ok(value !== undefined, "Expected value to be defined."),
  empty: (value: any): void =>
    uvu.ok(!value || value.length === 0 || Object.keys(value).length === 0),
  ...
  length: (value: string | unknown[], length: number): void =>
    uvu.is(value.length, length),
  ...
  matchesObject: (value: unknown, schema: ZodRawShape): void =>
    uvu.not.throws(() => z.object(schema).parse(value)),
  not: {
    ...uvu.not,
    containKey: (value: object, key: string): void =>
      assert.false(Object.keys(value).includes(key)),
    defined: (value: unknown): void =>
      uvu.ok(value === undefined, "Expected value not to be defined."),
    empty: (value: unknown[]): void => uvu.ok(Object.keys(value).length > 0),
    matchesObject: (value: unknown, schema: ZodRawShape): void =>
      uvu.throws(() => z.object(schema).parse(value)),
    undefined: (value: unknown): void =>
      uvu.ok(value !== undefined, "Expected value not to be undefined."),
  },
  ...
  rejects: async (
    callback: Function,
    ...expected: (uvu.Message | Constructable)[]
  ): Promise<void> => {
    try {
      await callback();
  ...
  snapshot: (name: string, value: unknown): void => {
    ...
    const updateSnapshots: boolean = process.argv.includes(
      "--update-snapshots",
    );
  ...
  startsWith: (value: string, prefix: string): void =>
    uvu.ok(value.startsWith(prefix)),
  string: (value: unknown): void => uvu.type(value, "string"),
  stringArray: (values: unknown[]): void =>
    uvu.ok(values.every((value) => typeof value === "string")),
  throws: (
    function_: Function,
    expects?: uvu.Message | RegExp | Function,
  ): void => {
  ...
  undefined: (value: unknown): void =>
    uvu.ok(value === undefined, "Expected value to be undefined."),
};
